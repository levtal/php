 ...
 <body>
  ...
  // Define the time zone based on the coordinated universal time
	...
	date_default_timezone_set('UTC');
         /* Echos the date      h : 12 hr format     H : 24 hr format
        i : Minutes   s : Seconds  u : Microseconds  a : Lowercase am or pm 
        l : Full text for the day  F : Full text for the month  j : Day of the month S : Suffix for the day st, nd, rd, etc. Y : 4 digit ear           */
...
EOD;
    ...
    When we call for a constant we don't use a $ and normally uppercase */
   define('PI', 3.1415926);
  ...
  // You can cast from 1 type to another like this
   ...
   // Use this shortcut when performing an operation 
   // using the same variable +=, -=, *=, /=, %=, .=
   $randNum = 5;
   echo $randNum += 5;// now $randNumis 10
   echo "</br></br>";
  // You can increment and decrement with this shortcut
  echo "++randNum = " . ++$randNum . "</br>";
  echo "randNum++ = " . $randNum++;
  echo "</br></br>";
   /* The reference operator (ampersan / &) can create a reference to a   variable so if one changes so does the  other */
...
// Other common array operators
 ...
 // You can trim white space with ltrim, rtrim, or trim
   ...
   echo 'ltrim=' . strlen(ltrim($randString)) . "</br>";
 ...
 // printf allows you to print formatted Strings to the screen
  ...
  printf ("decimal num = %.2f </br>", $decimalNum);
   // Other conversion codes  // b : integer to binary
   // c : integer to character // d : integer to decimal
   // f : double to float   // o : integer to octal
   // s : string to string  // x : integer to hexadecimal
 ...
 echo 'Does $biggestNum exist ';
  ...
  echo "</br>jjjjjjjjjjjjj</br>";
 // You can execute unix commands by surrounding with `s
 echo `ls -la`; // Unix or OSX
 //hell_exec() has been disabled for security reasons in /home/u643891464/public_html/p1/learnphp.php on line 287
 // echo `dir /w`; WINDOWS
 echo "</br>jjjjjjjjjjjj333333</br>";
